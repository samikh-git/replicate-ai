#!/usr/bin/env python3
"""Download AJR (2001) replication dataset and write data.csv."""
...
AJR_DATA_URL = (
    "https://economics.mit.edu/sites/default/files/inline-files/"
    "20100816_replication_dataset.dta"
)
DEFAULT_RAW = EXAMPLE_DIR / "raw" / "ajr_replication.dta"
DEFAULT_OUTPUT = EXAMPLE_DIR / "data.csv"


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--url", default=AJR_DATA_URL)
    ...
    if not (EXAMPLE_DIR / "paper.pdf").is_file():
        print("Next: add paper.pdf (Acemoglu, Johnson & Robinson 2001 AER).")
    return 0
